...
define([
  ...
  function(declare, lang, array, html, esriBundle, on, _WidgetsInTemplateMixin,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      //Mostramos las capas editables al abrir el widget  
      //------------------------------------------------
      var share = this.map.getLayer("SharePhotoLocation_8829");
      share.show();
      var share = this.map.getLayer("ShareRoutes_6510");
      share.show();
      //-------------------------------------------
        this._init();
      ...
      onClose: function() {
        ...
        //ocultamos las capas editables al cerrar el widget
        //-------------------------------------------------
        var share = this.map.getLayer("SharePhotoLocation_8829");
        share.hide();
        var share = this.map.getLayer("ShareRoutes_6510");
      ...
      //Funcion de loggin: Se ejecuta al intentar logearnos.
      funcionLogearse:function(){
          this.inherited(arguments);
          //Array de arrays con los nombres de usuario y contraseña de nuestros usuarios 
          this.userPass = [["Carlos","1234"],["Juli","123"]];
          //nombres de usuario y contraseña introducidos
          var usuario = usuarioInput.value;
          var pass = pssInput.value;
            //Bucle for que recorre el array de nombres de usuario y contraseña que tenemos y los compara con los introducidos
            for (var i = 0; i < this.userPass.length; i++) {
                  //En caso de que coincidan nombre de usuario y contraseña
                  if (this.userPass[i][0] == usuario && this.userPass[i][1] == pass) {
                    //Apagamos la venta de loggin y el usuario puede editar
                    document.getElementById("usuarioYpassDiv").style.display = "none";

                  }else{
                    //En caso de que el usuario no se logee correctamente se le avisa
